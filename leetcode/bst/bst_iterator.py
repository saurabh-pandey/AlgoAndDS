...
Could you implement next() and hasNext() to run in average O(1) time and use O(h) memory, where h 
...
class BSTIterator:
    def __init__(self, root):
        self.node_stack = []
        self.assemble_left_subtree_stack(root)
    ...
    def next(self):
        if not self.node_stack:
            ...
            currNode = self.node_stack.pop()
            self.assemble_left_subtree_stack(currNode.right)
            return currNode.val
    ...
    def assemble_left_subtree_stack(self, root):
        node = root
