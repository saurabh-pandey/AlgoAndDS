...
A string s is lexicographically sorted if for all valid i, s[i] is the same as or comes before s[i
+1] in the alphabet.
...
def countVowelStrings(n):
    ...
    prevCounts = [1 for _ in range(5)]
    totalSum = sum(prevCounts)
    for i in range(n - 1):
        counts[0] = totalSum
        for i in range(1, 5):
            counts[i] = counts[i - 1] - prevCounts[i - 1]
        prevCounts = counts[:]
        totalSum = sum(prevCounts)
    return totalSum